...
class ExtractGraphTFIDFConfig(BaseModel):
    threshold: float = 0.1
    min_df: int | float = 1
    max_df: float = 0.8
...
class GraphRagConfig(BaseModel):
    ...
    extract_graph_tfidf: ExtractGraphTFIDFConfig = Field(
        default_factory=ExtractGraphTFIDFConfig,
        description="TF-IDF-based graph extraction configuration"
    )

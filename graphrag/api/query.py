...
from graphrag.query.structured_search.local_search.weighted_multi_ppr import weighted_multisource_ppr
...
def log_used_documents(
    query: str,
    top_nodes_df: pd.DataFrame,
    document_text_dict: dict,
    output_path: str = "logs/used_documents.jsonl",
):
    ...
    records = []
    for rank, row in enumerate(top_nodes_df.itertuples(index=False), start=1):
        node_id = str(row.node)
        records.append({
            "rank": rank,
            "node_id": node_id,
            "score": float(row.score),
            "text_length": len(document_text_dict.get(node_id, "")),
        })
...
def compute_idf(term: str, df_dict: dict, N_docs: int) -> float:
...
def build_query_term_weights_idf_embedding(
    query: str,
    df_dict: dict[str, int],
    N_docs: int,
    analyzer,
    embed_fn,  # text -> np.ndarray
):
    terms = analyzer(query)
    if not terms:
        raise ValueError("Query has no valid terms")

    # embedding
    query_emb = embed_fn(query)
    term_embs = {t: embed_fn(t) for t in terms}

    raw_weights = {}

    for term in terms:
        idf = compute_idf(term, df_dict, N_docs)
        sim = cosine_sim(query_emb, term_embs[term])
        sim = max(sim, 0.0)

        raw_weights[term] = idf * sim

    # 正規化
    total = sum(raw_weights.values())
    if total == 0:
        # フォールバック（均等）
        return {t: 1 / len(terms) for t in terms}
...
def local_search_streaming(
    ...
    # -----------------------------
    # NEW: select top document nodes by weighted_multisource_ppr
    # -----------------------------
    try:
        # Build query_term_weights naively by splitting the query and giving equal weight
        # (user chose option A). You can replace this with a more advanced weighting function.
        query_term_weights = build_query_term_weights_idf_embedding(
            query=query,
            df_dict=df_dict,
            N_docs=N_docs,
            analyzer=analyzer,
        ...
        # relationships (pandas DataFrame) is provided to this outer function
        # weighted_multisource_ppr returns (top_nodes_df, full_ppr)
        top_nodes_df, _full_ppr = weighted_multisource_ppr(
            relationships,
            query_term_weights,
            teleport_prob=0.15,
            max_iter=100,
        ...
        # Build document_text_dict from text_units DataFrame.
        # text_units has columns ['id', 'text', ...] per your input.
        # Ensure keys are strings to match node ids used by relationships.target
        document_text_dict = {}
        # if text_units is a list of TextUnit objects, adapt accordingly.
        if isinstance(text_units, list):
            # TextUnit objects expected to have .id and .text attributes
            for tu in text_units:
                document_text_dict[str(tu.id)] = getattr(tu, "text", "")
        else:
            # assume pandas DataFrame as provided earlier
            for _, row in text_units.iterrows():
                document_text_dict[str(row["id"])] = row.get("text", "")

    except Exception:
        # if anything fails, fall back to empty top_nodes_df so system remains robust
        import pandas as _pd
        top_nodes_df = _pd.DataFrame(columns=["node", "score"])
        document_text_dict = {}

    doc_ids = set(document_text_dict.keys())  # text_units から作った辞書のキー
    top_nodes = top_nodes_df["node"].astype(str).tolist()
    hit = sum(n in doc_ids for n in top_nodes)

    log_used_documents(
        query=query,
        top_nodes_df=top_nodes_df,
        document_text_dict=document_text_dict,
    )
    ...
    search_engine = get_local_search_engine(
        ...
        callbacks=callbacks,
        extra_context_builder_params={
            "top_nodes_df": top_nodes_df,
            "document_text_dict": document_text_dict,
            # optionally override max_context_tokens:
            "max_context_tokens": getattr(config.local_search, "max_context_tokens", 5000),
        },
    )
    return search_engine.stream_search(query=query)

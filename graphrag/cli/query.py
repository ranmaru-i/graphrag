...
def run_local_search(
    ...
    # ★ 追加ここから
    df_stats: pd.DataFrame = dataframe_dict["df_stats"]
    corpus_stats: pd.DataFrame = dataframe_dict["corpus_stats"]
    tfidf_vectorizer: pd.DataFrame = dataframe_dict["tfidf_vectorizer"]
    

    df_dict = dict(zip(df_stats["term"], df_stats["df"]))
    N_docs = int(corpus_stats.iloc[0]["N_docs"])
    vectorizer = pickle.loads(tfidf_vectorizer.iloc[0]["blob"])
